      ...
      { 
        ...
        let arguments = List.map ~f:(fun a -> 
          if (String.contains a '=') then
            ...
            Some (NameValue (List.nth_exn a 0, List.nth_exn a 1)))
          else 
            if String.equal (String.strip a) "" then 
              None
            else
              Some (Field (String.strip a))
        ) arguments in
        let arguments = List.filter_map ~f:(fun x -> x) arguments in
